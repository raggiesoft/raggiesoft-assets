/**
 * ============================================================================
 * THE STARDUST PLAYER ENGINE (v3.0 - "Engine Room" Update)
 * ============================================================================
 * * CORE ARCHITECTURE:
 ...
 * across page reloads.
 * * 4. Multi-Artist Support (v3.0):
 * Updated DOM logic to handle the new "Artist" field in the UI, supporting
 * the Engine Room Radio mixed broadcast format.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        artist: document.getElementById('player-track-artist'),   // NEW: Artist Field
    ...
    // Check if modal exists first (to avoid errors on pages without it)
    let bsModal = null;
    if(dom.modalElement) {
    ...
    window.loadTrack = function(index, autoPlay = true) {
        ...
        dom.title.innerHTML = `
            ...
            <span class="text-muted fst-italic">Buffering...</span>
        `;
        if(dom.artist) dom.artist.textContent = track.artist; // Set Artist immediately
